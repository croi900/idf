...
namespace dirtree {
    ...
    public:
        void add_dir(const std::string &path, std::vector<std::string> &&files_in_dir) {
        ...
        std::vector<std::string> to_vector() const {
            ...
            for (const auto &d: dirs) {
                for (const auto &f: d.files) {
                    std::string full_path = d.path + "/" + f;
                    size_t s = 0;
                    try {
                        s = std::filesystem::file_size(full_path);
                    } catch (...) {


                    }
                    sized_files.push_back({s, std::move(full_path)});
                }
            }
            


            std::sort(sized_files.begin(), sized_files.end(), [](const auto& a, const auto& b) {
                return a.first > b.first;
            });
            
            std::vector<std::string> result;
            result.reserve(total_files);
            for (auto& p : sized_files) {
    ...
    inline void list_files_serial_internal(const std::string &root_path, FileCollection &local_buffer) {
        ...
        while (!stack.empty()) {
                    ...
                    if (name != "." && name != "..") {
                        if (d->d_type == DT_DIR) {
                            ...
                            if (dot_pos != std::string_view::npos) {
                                if (is_text_file(name.substr(dot_pos))) {
            ...
            if (!local_files.empty()) {
                local_buffer.add_dir(path, std::move(local_files));
            }
        }
    }
    ...
    inline void list_files_hpx_internal(std::string path,
        ...
        if (!local_files.empty()) {
            tls_buffers[thread_id].add_dir(path, std::move(local_files));
        }
        ...
        for (auto &subdir: subdirs) {
            if (active_tasks.load(std::memory_order_relaxed) < max_tasks) {
                ...
                hpx::post(
                    [subdir = std::move(subdir), &tls_buffers, &active_tasks, max_tasks, &completion_promise
                    ]() mutable {
    ...
    inline FileCollection file_list(const std::filesystem::path &p, const int max_tasks = 2000) {
        ...
        for (auto &buf: tls_buffers) {
            result.merge(std::move(buf));
        }
