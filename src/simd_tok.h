...
namespace idf {
    ...
    inline void tokenize_simd_avx512(std::string_view content, uint64_t base_offset, std::vector<TokenInfo>& tokens) {
        const char* data = content.data();
        size_t size = content.size();
        size_t i = 0;
        ...
        for (; i + 63 < size; i += 64) {
            ...
            uint64_t mask = (uint64_t)(
                _mm512_cmpeq_epi8_mask(chunk, space)    |
            ...
            while (current_bit < 64) {
                if (!in_word) {
                    uint64_t remaining = (~mask) >> current_bit;
                    if (remaining == 0) break;
                    uint64_t skip = __builtin_ctzll(remaining);
                    current_bit += skip;
                    last_word_start = i + current_bit;
                    in_word = true;
                } else {
                    ...
                    uint64_t skip = __builtin_ctzll(remaining);
                    current_bit += skip;
                    tokens.push_back({base_offset + last_word_start,
                                      content.substr(last_word_start, (i + current_bit) - last_word_start)});
                    in_word = false;
                }
            }
        }
